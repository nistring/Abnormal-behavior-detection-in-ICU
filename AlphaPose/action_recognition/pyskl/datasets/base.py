...
from ..core import mean_average_precision, mean_class_accuracy, top_k_accuracy, mean_score
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate(self,
        ...
        allowed_metrics = ['top_k_accuracy', 'mean_class_accuracy', 'mean_average_precision', 'mean_score']
        ...
        for metric in metrics:
            ...
            if metric == 'mean_score':
                mean_s = mean_score(results, gt_labels)
                log_msg = f'\nmean_score\t{mean_s:.4f}'
                print_log(log_msg, logger=logger)
                continue
